...
async function testDeploy() {
  const projectRoot = join(__dirname, '..', 'workflows-backend');
  ...
  const localRoot = join(__dirname, '..', 'tests', 'test-mcp-local');
  const localWorkerPath = join(localRoot, 'worker.mjs');
  const localBundlesRoot = join(localRoot, 'bundles');
  const projectBundlesRoot = join(__dirname, '..', 'workflows-backend', 'bundles');
