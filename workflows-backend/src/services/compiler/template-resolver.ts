/**
 * Template Resolver - Resolves template expressions for visualization
 */
...
export class TemplateResolver {
  /**
   * Resolve template expression using graph context
   */
        ...
        for (const [key, value] of Object.entries(config)) {
          if (typeof value === "string" && value.includes("{{")) {
            const result = yield* _(
              TemplateResolver.resolveTemplate(value, graphContext, node.id)
            );
            ...
            const nestedResult = yield* _(
              TemplateResolver.resolveNestedTemplates(value, graphContext, node.id)
            );
  ...
  /**
   * Resolve templates in nested objects
   */
  private static resolveNestedTemplates(
    obj: unknown,
    graphContext: GraphContext,
    nodeId: string
    ...
    return Effect.gen(function* (_) {
      if (Array.isArray(obj)) {
        ...
        for (const item of obj) {
          if (typeof item === "string" && item.includes("{{")) {
            const result = yield* _(
              TemplateResolver.resolveTemplate(item, graphContext, nodeId)
            );
            ...
            const nestedResult = yield* _(
              TemplateResolver.resolveNestedTemplates(item, graphContext, nodeId)
            );
      ...
      for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
        if (typeof value === "string" && value.includes("{{")) {
          ...
          const nestedResult = yield* _(
            TemplateResolver.resolveNestedTemplates(value, graphContext, nodeId)
          );
